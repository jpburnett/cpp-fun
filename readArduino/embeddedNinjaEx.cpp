...
void WriteToFile(std::ofstream &myFile, std::string dataIn)
{
...
int main()
{
    ...
    myfile << "A0,A1,A2,A3" << std::endl;

    // Open the serial port. Change device path as needed (currently set to an standard FTDI USB-UART cable type device)
    int serial_port = open("/dev/cu.usbserial-1420", O_RDWR);
    ...
    // This may put it in an infinite loop, but whatever makes it work.....right?
    while (true)
    {
        int num_bytes = read(serial_port, &read_buf, sizeof(read_buf));
        WriteToFile(myfile, read_buf);
